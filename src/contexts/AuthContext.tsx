...
import { saveUserToMongoDB } from "../lib/mongodb"; // Import the MongoDB function
...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  async function signUp(email: string, password: string) {
    ...
    await setDoc(doc(db, "users", result.user.uid), userDataObj);
    
    // Save user to MongoDB as well
    try {
  ...
  async function signInWithGoogle() {
    ...
    // Save or update user in MongoDB regardless
    try {
      await saveUserToMongoDB({
        uid: result.user.uid,
        email: result.user.email || '',
